...
$query=mysql_query("select b.*,c.name as bookcasename,p.pubname,t.typename from tb_bookinfo b 
		left join tb_bookcase c on b.bookcase=c.id 
		join tb_publishing p on b.ISBM=p.ISBN 
...
?>
  <tr>
    ...
    <td style="padding:5px;">&nbsp;<?php echo $result[storage];?></td> 
    <td style="padding:5px;">&nbsp;<?php echo $result[bookin];?></td> 
    </tr>
<?php
...
$f=$_POST[f];
$key1=$_POST[key1];
$sql=mysql_query("select b.*,c.name as bookcasename,p.pubname,t.typename 
		from tb_bookinfo b left join tb_bookcase c on b.bookcase=c.id 
		join tb_publishing p on b.ISBM=p.ISBN 
		join tb_booktype t on b.typeid=t.id where $f like '%$key1%'");
